...
| 设计决策 | 不同选择 | 代表项目 |
|:---|:---|:---|
| **循环控制** | 简单 `while` vs 状态机驱动 | Codex vs Gemini CLI |
| **状态回滚** | 不实现 vs 命令级 Checkpoint | OpenAI vs Kimi CLI |
| **上下文压缩** | 滑动窗口 vs 三层分层架构 | Codex vs Gemini CLI |
| **安全隔离** | 原生沙箱 vs 进程隔离 | Codex (Rust) vs SWE-agent |
| **工具并发** | 顺序执行 vs 并行调用 | Kimi CLI vs OpenCode |
...
- 学习状态持久化、Checkpoint 回滚机制（Kimi CLI vs SWE-agent 两种思路）
...
| **Memory 管理** | [codex](./docs/codex/07-codex-memory-context.md) · [gemini-cli](./docs/gemini-cli/07-gemini-cli-memory-context.md) · [kimi-cli](./docs/kimi-cli/07-kimi-cli-memory-context.md) · [comm 对比](./docs/comm/07-comm-memory-context.md) | Token 超限怎么办？滑动窗口 vs 分层架构？ |
| **Safety 控制** | [codex](./docs/codex/10-codex-safety-control.md) · [gemini-cli](./docs/gemini-cli/10-gemini-cli-safety-control.md) · [kimi-cli](./docs/kimi-cli/10-kimi-cli-safety-control.md) | 如何安全执行用户代码？权限如何分级？ |
