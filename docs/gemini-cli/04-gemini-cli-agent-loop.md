...
│  │  ├── 重置状态(loop detector等)          │                        │
│  │  ├── BeforeAgent Hook 检查              │ ──► 可 stop/block      │
│  │  └── processTurn()                      │                        │
│  └────────┬────────────────────────────────┘                        │
└───────────┼─────────────────────────────────────────────────────────┘
            │
...
│  │  └── 生成 functionResponse              │                        │
│  │         ↓                               │                        │
│  │  有 pending tool calls?                 │                        │
│  │         ├────Yes────────────────────────┼──► 递归                │
│  │         │   submitQuery(isContinuation  │      sendMessageStream │
│  │         │              =true)           │      (新一轮)          │
│  │         └────No                         │                        │
│  │             Finished 收敛 ◄─────────────┼──► 循环结束            │
│  └─────────────────────────────────────────┘                        │
