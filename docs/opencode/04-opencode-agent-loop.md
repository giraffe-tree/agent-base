...
└───────────┼─────────────────────────────────────────────────────────┘
            ...
            ▼ (noReply? 直接返回)
┌─────────────────────────────────────────────────────────────────────┐
│  主循环层: loop() while(true)                                        │
│  ┌─────────────────────────────────────────┐                        │
│  │ 每次迭代:                               │                        │
│  │                                         │                        │
│  │  1. 读取消息流 (MessageV2.stream)       │ ──► 过滤已压缩历史     │
│  │     ├── 定位 lastUser                 │ ──► 最新用户消息       │
│  │     ├── 定位 lastFinished             │ ──► 最近完成 assistant │
│  │     └── 定位 tasks                    │ ──► 待处理任务         │
│  │                                         │                        │
│  │  2. 退出条件检查                        │ ◄── 迭代开头           │
│  │     lastAssistant.finish 存在?          │                        │
│  │     AND finish ≠ "tool-calls"/"unknown" │                        │
│  │     AND lastUser.id < lastAssistant.id  │ ──► Yes: return 结束   │
│  │                                         │                        │
│  └────────┬────────────────────────────────┘                        │
└───────────┼─────────────────────────────────────────────────────────┘
            │
...
└──────────────────────────────────────────────────────────────────────┘
    ...
    │ 步骤链:                                                      │
    │                                                             │
    │  1. Agent 解析 + steps 上限检查                              │
    │           ↓                                                │
    │  2. insertReminders() 注入提示词                            │
    │           ↓                                                │
    │  3. resolveTools() 合并并过滤工具                           │
    │     • ToolRegistry 工具                                    │
    │     • MCP 工具                                             │
    │     • 按 agent 权限过滤                                     │
    │           ↓                                                │
