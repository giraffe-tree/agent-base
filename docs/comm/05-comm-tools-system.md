...
```text
AgentLoop.run()                    [kimi-cli/kimi/core/agent.py:200]
  -> get_tools()                   [tools/registry.py:50]
    -> ToolRegistry.list()         [tools/registry.py:75]
      -> convert_to_schema()       [tools/schema.py:30]
        - 遍历所有工具
        - 转换为 JSON Schema

AgentLoop.handle_tool_call()       [kimi-cli/kimi/core/agent.py:250]
  -> registry.get_handler()        [tools/registry.py:100]
    -> handler.execute()           [tools/handlers/base.py:40]
      - 权限检查
      - 实际执行
      - 结果格式化
```
...
| Codex | Rust Trait，编译期安全 | 企业级应用，性能和安全优先 |
| Gemini CLI | Kind 分类，权限内聚 | 需要精细权限控制的生产环境 |
| OpenCode | Zod 校验，现代 TS 生态 | 追求开发效率和类型安全的项目 |
...
| 执行超时 | 工具执行超过配置时间 | 各项目 timeout 配置 |
| 输出超限 | 结果超过最大 token 限制 | `tool/truncation.ts` |
...
| 权限拒绝 | 记录拒绝原因，询问用户是否重试 | `gemini-cli/packages/core/src/tools/kind.ts` |
| 执行超时 | 返回部分结果或超时错误 | 各项目 timeout handler |
| 文件不存在 | 返回 FileNotFound，LLM 可尝试其他路径 | `codex-rs/core/src/tools/read_file.rs` |
| 输出截断 | 标记 truncated=true，提示 LLM 内容不完整 | `tool/truncation.ts` |
...
*基于版本：2026-02-08 源码分析 | 最后更新：2026-02-25*
