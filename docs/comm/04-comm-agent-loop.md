...
Agent Loop 是 Code Agent 的控制核心，让 LLM 从"一次性回答"变成"多轮执行"。
...
| `Loop Detector` | 检测重复模式，防止无限循环 | `gemini-cli/packages/core/src/core/client.ts:789` |
...
def run(self, task: str):
    ...
    while not self.terminated:
        # sweagent/agent/agents.py:328
        step_result = self.step()
        # sweagent/agent/agents.py:336
...
| **无限循环** | loopDetector 检测并强制终止 | `gemini-cli/packages/core/src/core/client.ts:789` |
