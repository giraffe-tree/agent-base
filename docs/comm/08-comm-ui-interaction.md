...
| 项目 | UI 技术选型 | 核心特点 |
|-----|------------|---------|
| Codex | **Ratatui TUI**（对比 Gemini CLI 的 Node.js 自定义） | 字符级流式渲染，富文本终端体验 |
| Gemini CLI | **Event Emitter 事件流**（对比 Kimi CLI 的 Wire 协议） | Token 级流式，内联工具卡片 |
| Kimi CLI | **Wire 协议解耦**（对比 OpenCode 的 Bus 总线） | 事件级粒度，Turn/Step 状态反馈 |
| OpenCode | **Bus 事件总线 + Part 状态**（对比 Codex 的 Channel） | Part 级更新，支持 Web UI |
| SWE-agent | **Python logging**（无流式） | 批量输出，轨迹保存 |
...
| 组件 | 职责 | 典型实现 |
|-----|------|---------|
| `事件接收器` | 接收 LLM 流式输出、工具执行事件 | Codex: Channel / Gemini: EventEmitter |
| `渲染引擎` | 将事件转换为终端可见的输出 | Ratatui / rich / ink |
| `状态管理` | 维护当前对话状态、用户确认状态 | 状态机 / 原子状态 |
| `输入处理器` | 处理用户键盘输入、确认响应 | 异步输入监听 |
...
| 组件 | 文件路径 | 说明 |
|-----|---------|------|
| TUI 入口 | `codex-rs/tui/src/lib.rs` | TUI 主循环实现 |
| 对话区域 | `codex-rs/tui/src/chat_area.rs` | 消息渲染与流式输出 |
| 事件处理 | `codex-rs/tui/src/event.rs` | 键盘/鼠标事件处理 |
...
| 组件 | 文件路径 | 说明 |
|-----|---------|------|
| Stream Hook | `packages/cli/src/ui/hooks/useGeminiStream.ts` | 流式数据 Hook 封装 |
| 事件定义 | `packages/core/src/core/events.ts` | 流事件类型定义 |
| UI 组件 | `packages/cli/src/ui/` | 终端 UI 组件 |
...
| 组件 | 文件路径 | 说明 |
|-----|---------|------|
| Wire 协议 | `kimi-cli/src/kimi_cli/wire.py` | Agent-UI 通信协议 |
| 事件定义 | `kimi-cli/src/kimi_cli/events.py` | UI 事件类型 |
| 渲染逻辑 | `kimi-cli/src/kimi_cli/render.py` | 终端渲染实现 |
...
| 组件 | 文件路径 | 说明 |
|-----|---------|------|
| 事件总线 | `packages/opencode/src/bus.ts` | 全局事件总线 |
| Part 更新 | `packages/opencode/src/session/part.ts` | 消息 Part 状态管理 |
| UI 组件 | `packages/opencode/src/ui/` | UI 组件实现 |
...
| 组件 | 文件路径 | 说明 |
|-----|---------|------|
| 日志配置 | `sweagent/utils/log.py` | 日志系统配置 |
| 轨迹保存 | `sweagent/agent/agents.py` | 执行轨迹记录 |
...
| 项目 | 事件系统 | 核心特点 | 代码位置 |
|-----|---------|---------|---------|
| Codex | Channel (Actor) | 消息驱动，类型安全 | `codex-rs/tui/src/event.rs` |
| Gemini CLI | Event Emitter | 标准 JS 事件模式 | `packages/core/src/core/events.ts` |
| Kimi CLI | Wire 协议 | Agent-UI 解耦 | `kimi-cli/src/kimi_cli/wire.py` |
| OpenCode | Bus 总线 | 发布订阅，支持多监听 | `packages/opencode/src/bus.ts` |
| SWE-agent | 回调函数 | 简单直接 | `sweagent/agent/agents.py` |

### 5.3 关键调用链

**Codex 流式渲染调用链**：
```text
main()                    [codex-rs/tui/src/main.rs]
  -> run_tui()             [codex-rs/tui/src/lib.rs]
    -> handle_event()      [codex-rs/tui/src/event.rs]
      -> render_stream()   [codex-rs/tui/src/chat_area.rs]
        - 逐字符更新内容
        - 自动滚动到底部
```
...
**Gemini CLI 事件处理链**：
```text
submitQuery()             [packages/cli/src/index.ts]
  -> useGeminiStream()     [packages/cli/src/ui/hooks/useGeminiStream.ts]
    -> processEvents()     [packages/core/src/core/events.ts]
      - Content 事件 → 文本渲染
      - Tool 事件 → 工具卡片
```
...
**Kimi CLI 的选择**：
- 代码依据：`kimi-cli/src/kimi_cli/wire.py`
...
| 终止原因 | 触发条件 | 处理方式 |
|---------|---------|---------|
| 用户主动退出 | Ctrl+C / 输入 exit | 清理资源，保存状态 |
| 会话超时 | 长时间无操作 | 提示用户，自动断开 |
| LLM 连接断开 | 网络异常 | 重试机制，降级提示 |
| 终端尺寸变化 | 窗口调整 | 重新渲染布局 |
...
| 项目 | 文件路径 | 说明 |
|-----|---------|------|
| Codex | `codex-rs/tui/src/lib.rs` | TUI 入口 |
| Gemini CLI | `packages/cli/src/index.ts` | CLI 入口 |
| Kimi CLI | `kimi-cli/src/kimi_cli/main.py` | 渲染入口 |
| OpenCode | `packages/opencode/src/main.ts` | CLI 入口 |
| SWE-agent | `sweagent/agent/agents.py` | Agent 主循环 |

### 8.2 流式输出

| 项目 | 文件路径 | 说明 |
|-----|---------|------|
| Codex | `codex-rs/tui/src/chat_area.rs` | 字符级流式渲染 |
| Gemini CLI | `packages/cli/src/ui/hooks/useGeminiStream.ts` | Stream Hook |
| Kimi CLI | `kimi-cli/src/kimi_cli/render.py` | 流式渲染 |
| OpenCode | `packages/opencode/src/session/processor.ts` | Part 更新 |
| SWE-agent | `sweagent/utils/log.py` | 日志输出 |

### 8.3 事件系统

| 项目 | 文件路径 | 说明 |
|-----|---------|------|
| Codex | `codex-rs/tui/src/event.rs` | 事件处理 |
| Gemini CLI | `packages/core/src/core/events.ts` | 事件定义 |
| Kimi CLI | `kimi-cli/src/kimi_cli/events.py` | 事件定义 |
| OpenCode | `packages/opencode/src/bus.ts` | 事件总线 |
| SWE-agent | `sweagent/agent/agents.py` | 回调处理 |

### 8.4 输入处理

| 项目 | 文件路径 | 说明 |
|-----|---------|------|
| Codex | `codex-rs/tui/src/input.rs` | 键盘输入处理 |
| Gemini CLI | `packages/cli/src/ui/input.ts` | 输入组件 |
| Kimi CLI | `kimi-cli/src/kimi_cli/input.py` | 用户输入 |
| OpenCode | `packages/opencode/src/ui/input.ts` | 输入处理 |
