...
UI 层决定用户"感知" Agent 的方式。核心设计问题是：是否流式输出（逐字显示）、工具执行是否可见、用户如何确认危险操作。流式输出是所有项目的标配；差异在于"显示什么"和"如何显示"。

---

## 设计动机：为什么 Agent UI 比普通 CLI 复杂？
...
这要求 UI 层能处理**并发的多个事件流**，而不是简单的请求-响应。

---

## 工程取舍：流式粒度 vs 实现复杂度

| UI 方案 | 流式粒度 | 工具可视化 | 实现复杂度 |
|---------|----------|------------|------------|
| 纯文本日志（SWE-agent）| 无（批量） | 命令文本 | 最低 |
| TUI（Codex）| 字符级 | 实时进度 | 高 |
| 富文本流（Gemini CLI）| 字符级 | 折叠展示 | 中 |
| 结构化事件流（Kimi CLI）| 事件级 | Wire 协议 | 中 |
| 组件化 UI（OpenCode）| 字符级 | Part 状态 | 高 |
