...
一句话定义：Qwen Code 的 MCP 集成是**基于官方 MCP SDK 的多传输层自动回退 + 自动 OAuth 发现 + 服务器级信任模型**的外部工具接入框架，通过 `McpClientManager` 统一管理多个 MCP 服务器的连接、发现和生命周期。

Qwen Code 的核心取舍：**官方 MCP SDK 原生集成 + 自动 OAuth 发现 + 完全限定名命名空间**（对比 Gemini CLI 的多传输自动回退、Codex 的 Rust 原生实现、Kimi CLI 的 ACP 桥接）
...
| `discoverMcpTools` | 发现所有配置服务器的工具 | `packages/core/src/tools/mcp-client.ts:486` |
...
| `DiscoveredMCPTool` | MCP 工具的包装，集成审批 | `packages/core/src/tools/mcp-tool.ts:329` |
| `generateValidName` | 生成符合 Gemini API 的工具名 | `packages/core/src/tools/mcp-tool.ts:547` |
...
```typescript
// packages/core/src/tools/mcp-tool.ts:329-387
...
```typescript
// packages/core/src/tools/mcp-tool.ts:117-152
  ...
  -> Promise.all(discoveryPromises)
    -> connectAndDiscover()        [mcp-client.ts:548]
      ...
      -> connectToMcpServer()      [mcp-client.ts:798]
        ...
        -> client.connect()
        -> 设置 onerror 处理器
      -> discoverPrompts()         [mcp-client.ts:694]
      -> discoverTools()           [mcp-client.ts:624]
        -> mcpToTool()             [@google/genai]
        -> isEnabled()             [mcp-client.ts:1407]
        -> new DiscoveredMCPTool() [mcp-tool.ts:329]
          - generateValidName()    [mcp-tool.ts:547]
...
| **审批控制** | 服务器级 trust + 白名单 | Policy Engine + trust | 白名单/黑名单 | 运行时审批 | 配置 enabled |
...
| 项目 | 核心差异 | 适用场景 |
|-----|---------|---------|
| **Qwen Code** | 官方 SDK + 自动 OAuth 发现 + Roots 支持 | 需要与官方 MCP 协议完全兼容，追求稳定性的场景 |
...
| 工具调用超时 | 超过 mcpTimeout 配置 | `mcp-tool.ts:216` |
| 审批拒绝 | 用户拒绝工具调用 | `mcp-tool.ts:149` |
...
| 工具发现 | `packages/core/src/tools/mcp-client.ts` | 624 | discoverTools() 函数 |
| Prompt 发现 | `packages/core/src/tools/mcp-client.ts` | 694 | discoverPrompts() 函数 |
| 工具启用检查 | `packages/core/src/tools/mcp-client.ts` | 1407 | isEnabled() 函数 |
| 工具包装 | `packages/core/src/tools/mcp-tool.ts` | 329 | DiscoveredMCPTool 类 |
| 工具调用 | `packages/core/src/tools/mcp-tool.ts` | 97 | DiscoveredMCPToolInvocation 类 |
| 审批检查 | `packages/core/src/tools/mcp-tool.ts` | 117 | shouldConfirmExecute() 方法 |
| 执行工具 | `packages/core/src/tools/mcp-tool.ts` | 176 | execute() 方法 |
| 名称生成 | `packages/core/src/tools/mcp-tool.ts` | 547 | generateValidName() 函数 |
| 内容转换 | `packages/core/src/tools/mcp-tool.ts` | 474 | transformMcpContentToParts() 函数 |
