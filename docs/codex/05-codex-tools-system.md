...
**新增功能**：
- Agent Jobs 批量处理系统支持 Map-Reduce 风格工作流，`spawn_agents_on_csv` 工具实现 CSV 批量处理，SQLite 持久化存储，exec 模式带进度条 UI
- `request_user_input` 工具现已在 Default 协作模式下可用（此前仅在 Plan 模式下可用）
...
| **BatchJobHandler** | **Agent Jobs 批量处理 Handler** | **`tools/handlers/agent_jobs.rs:34`** |
...
**Agent Jobs 设计意图**：
- 代码依据：`agent_jobs.rs:36-40` 的并发常量和 `state/runtime/agent_jobs.rs` 的状态管理
...
| **Agent Job 超时** | **超过 max_runtime_seconds** | **`agent_jobs.rs:40`** |
| **Agent Job 取消** | **任务被取消或父 Agent 终止** | **`state/runtime/agent_jobs.rs:271`** |
...
| 边界情况 | 处理策略 | 代码位置 |
|---------|---------|---------|
| 子 Agent 失败 | 重试机制，最多 3 次 | `agent_jobs.rs` |
| 线程深度超限 | 拒绝创建，返回错误 | `agent_jobs.rs:1` (exceeds_thread_spawn_depth_limit) |
| 任务取消 | 标记 Cancelled，停止调度 | `state/runtime/agent_jobs.rs:271` |
| 结果上报超时 | 30 分钟默认超时 | `agent_jobs.rs:40` |
...
| **Agent Jobs Handler** | **`tools/handlers/agent_jobs.rs`** | **34** | **BatchJobHandler 实现** |
| **Agent Jobs 状态** | **`state/src/model/agent_job.rs`** | **1** | **数据模型定义** |
| **Agent Jobs 存储** | **`state/src/runtime/agent_jobs.rs`** | **1** | **SQLite 操作** |
| **进度条 UI** | **`exec/src/event_processor_with_human_output.rs`** | **968** | **render_agent_job_progress** |
