...
一句话定义：`ThinkPart` 是 Kimi CLI 中用于捕获和持久化 LLM 推理过程的内容组件，支持思考内容的加密签名和合并操作。

Kimi CLI 的核心取舍：**保留完整推理内容以支持 D-Mail 时间旅行机制和状态回滚**（对比 Codex 的流式思考处理、Gemini CLI 的无持久化思考内容）
...
| 组件 | 职责 | 代码位置 |
|-----|------|---------|
| `ThinkPart` | 封装 LLM 推理内容，支持加密签名 | `packages/kosong/src/kosong/message.py:91` |
| `ContentPart` | 内容组件抽象基类，提供合并接口 | `packages/kosong/src/kosong/message.py:16` |
| `DenwaRenji` | D-Mail 管理器，处理时间旅行消息 | `src/kimi_cli/soul/denwarenji.py:16` |
| `Context` | 对话上下文管理，支持 checkpoint 回滚 | `src/kimi_cli/soul/context.py:16` |
| `SimpleCompaction` | 上下文压缩，选择性保留 ThinkPart | `src/kimi_cli/soul/compaction.py:42` |
...
```python
# src/kimi_cli/soul/kimisoul.py:428-451
# 处理 pending D-Mail
...
# src/kimi_cli/soul/compaction.py:112-114
content.extend(
